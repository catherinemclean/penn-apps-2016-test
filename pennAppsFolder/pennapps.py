...
def make_bar_chart(choice):
    graphs = [
              dict(
                   data=[
                         dict(
                              x = ['% over 25 with HS diploma', '% with Bachelor\'s or higher', '% under 65 with a disability', '% without health insurance', '% in poverty'],
                              y = countsDct[choice],
                              type = 'bar'
                              ),
                         ],
                   layout=dict(
                               title = "Census data from "+ choice
                               )
                   )
              ]
...
class ChoiceForm(Form):
    ...
    choice = SelectField(u'Departments', choices=[("blank", "Make a choice"), ('Alabama','Alabama'),
('Alaska','Alaska'),
('Arizona','Arizona'),
('Arkansas','Arkansas'),
